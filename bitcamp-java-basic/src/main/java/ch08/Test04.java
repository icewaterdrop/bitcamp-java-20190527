...
/*
 * # 클래스 로딩 1) 클래스를 외부 저장소에서 내부 저장소인 RAM으로 로딩한다. 2) 클래스의 바이트코드 유효성을 검사한다. 3) 클래스 내부에서 사용하는 모든
 * 이름(변수명, 메서드명, 클래스명 등) 목록을 준비한다. 4) 클래스 필드를 찾아 생성한다. 5) 스태틱 블록이 있으면 순서대로 실행한다.
 * 
 */
