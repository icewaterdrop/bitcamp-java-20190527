...
public interface Printer {
  ...
  // 기존에 이 인터페이스의 규칙에 따라 작성한 모든 클래스들에
  // 컴파일 오류가 발생할 것이다.
  // 왜? 새 규칙(메서드)이 추가 되었기 때문이다.
  // 새 규칙이 추가 되었으면 기존 클래스들도 새 규칙을 구현해야 한다.
  //
  ...
  // 새규칙을 추가할  수 있다.
  // => 디폴트 메서드를 추가 할 때 간단하게 코드를 작성할 수 있지만
  //    클래스처럼 뭔가 진짜 일을 하는 코드를 작성하는것은 좋지 않다.
