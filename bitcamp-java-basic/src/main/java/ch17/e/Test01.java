...
public class Test01 {
  ...
  public static void main(String[] args) {
    ...
    // Sorter 클래스의 sort() 메서드가 추상 메서드이기 때문에
    // Sorter 클래스를 상속받은 서브 클래스가 무엇이든 간에
    // sort() 메서드가 반드시 구현되어 있을 것이다.
    // 따라서 display() 메서드에서는 파라미터로 넘어오는 Sorter가 
    // BubbleSort 인지, QuickSort인지, MergerSort인지 따질 필요 없이,
    // 그냥 서브 클래스가 구현한 sort()를 호출 하면 된다.
    // 
  ...
  static void display(Sorter sorter, int[] values) {
    ...
    // 어? 강사님 Sorter클래스의 sort() 메서드는 추상 메서드인데
