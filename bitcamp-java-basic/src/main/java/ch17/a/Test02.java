...
public class Test02 {
  ...
  public static void main(String[] args) {
    ...
    A obj4 = new My2();
    obj4.m1(); // m1() 은 A 클래스에 있는 메서드이다. 호출 할 수 있다
               // 다만 obj4에 저장된 인스턴스 (주소)는 실제 My2클래스이기 때문에
    ...
    // obj4.m3(); // 컴파일 오류!
    // obj4 레퍼런스는 A클래스의 레퍼런스이기 때문에 문법적으로 m3() 메서드가 없어서 호출 할 수 없다.
    // obj4가 어떤 클래스의 인스턴스를 가리키는 가는 중요한 것이 아니다.
    // 일단 메서드를 호출 할때 레퍼런스의 클래스의 조건에 맞는 메서드만 호출할 수 있다.


    // obj4 에 My2 인스턴스를 저장한것이 확실하다면
    // 컴파일러 에게 개발자가 직접 알려줘야 한다.
