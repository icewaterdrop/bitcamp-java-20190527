...
//LinkedList : Node 클래스를 중첩 클래스(static nested class)로 만들기
...
//    }
  ...
  // LinkedList 에서 사용하는 클래스라면 굳이 패키지 멤버 클래스로 만들 필요가 없다.
  // LinkedList 안에 선언하여 중첩 클래스로 정의하는 것이
  // 소스 코드의 유지보수에 좋다
  // 외부에 직접 노출 되지 않기 떄문에 쓸데 없는 클래스를 감추는 효과도 있다.
