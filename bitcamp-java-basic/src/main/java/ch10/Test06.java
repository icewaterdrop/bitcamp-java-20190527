...
class Monitor4 {
  ...
  //  예) new 클래스명(호출할 생성자에 전달할 파라미터 값)
  // => 다음과 같이 파라미터의 값을 주지 않으면 파라미터 값을 안받는 생성자가 호출 된다.
  //  예) Monitor4()
  // => 파라미터 값을 안 받는 생성자 (파라미터가 없는 생성자)를 " 
  //    기본 생성자(default constructor)" 부른다.
  // => new 명령을 실행하여 인스턴스를 생성 할 때는 반드시 호출될 생성자를 지정해야 한다.
  //  예) new 클래스명();
...
public class Test06 {
  public static void main(String[] args) {
    // 모니터 인스턴스 생성
    // 1) Monitor4 설계도에 따라 인스턴스 필드를 Heap 에 생성한다.
    // 2) 필드의 초기화 문장을 실행한다.
    //   예) int bright = 10 ;
    // 3) 인스턴스 초기화 블록을 실행한다.
    //   예)
    ...
    //        ...
    //       }
    // 4) 파라미터를 안받는 생성자(dufault constructor)를 호출한다.
    //  예 ) Monitor4() {...}
    //
