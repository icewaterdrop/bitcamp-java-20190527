...
public class Test01_2 {
  ...
  public static void main(String[] args) {
    ...
    // 메서드 호출자가 예외를 처리하지않으면 그 상위호출자에게 전달된다
    // 그 상위 호출자가 예외를 처리하지 않으면 그 상위의 상위 호출자 에게 전달된다.
    // 이런식으로 계속 전달 되다 보면 main() 메서드까지 전달 되고,
    // main()에서도 예외를 처리하지 않으면 최종적으로 JVM에게 전달된다
    // JVM이 예외를 받으면 그 즉시 프로그램을 멈춘다.
    // 따라서 try ~ catch 사용을 강요받지 않더라도
    // RuntumeException 예외를 처리하는것이 JVM을 멈추지 않게 하는 것이다.
    //
    // 예 :
    //
    ...
    // 그렇다고 특정 예외 조건에서 예외가 발생되지 않는것은 아니다.
    // 다만 이 메서드에서 처리하지 않으면
    // 이 메서드를 호출한 쪽에 예외를 자동으로 전달한다.
    // main()을 호출한 것은 JVM이기 떄문에
    // main()에서 예외를 처리하지 않으면 JVM에게 전달되고
    // JVM은 예외를 받는 즉시 실행을 멈춘다

    result = divide(100, 0); // 예외발생 main()호출자에게 전달 . 즉 JVM에 전달. 즉시종료
    System.out.println(result);
