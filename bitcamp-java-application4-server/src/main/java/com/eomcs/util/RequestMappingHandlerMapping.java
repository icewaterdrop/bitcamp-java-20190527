...
public class RequestMappingHandlerMapping {
  ...
  public static class RequestHandler {
    ...
    public Object invoke(
      ...
      // 페이지 컨틀롤러가 작업한 결과를 받을 바구니를 준비한다.
      ...
      // model 객체에는 request handler 가 담은 값이 보관 되어있다
      // 여기에 request handler 의 리턴 값(JSP URl) 도 함께 보관한다.
    ...
    private Object getPojoValue(
      ...
      // 클라이언트가 보낸 데이터를 담을 POJP 객체를 생성한다.
      // => 기본 생성자를 호출하여 인스턴스를 초기화 시킨다.
      ...
      // 클라이언트가 보낸 데이터 중에서 새터 메서드에 넘겨줄 요청 데이터가 있다면 호출한다. 
      for (Method m : methods) {
        // 매서드 이름에서 프로퍼티 명을 추출한다.
        // => 예 : // setCreatedDate() => "c"+ "reatedDate" = createdDate 
        String proName = m.getName().substring(3,4).toLowerCase() + 
            m.getName().substring(4); 


        log.debug("     " + proName);
        // 세터를 호출할 때 넘겨줄 값을 담을 변수를 준비
        Object value = null;
        ...
        // 세터에 넘겨줄 값을 준비하지 못했으면 다음 메서드로 넘어간다.
        if (value == null) // 프로퍼티와 일치하는 값을 찾지 못했으면 다음세터 메서드를 조사한다.
          continue;

        // 클라이언트가 보낸 값 중에서 파라미터 이름과 일치하는 
