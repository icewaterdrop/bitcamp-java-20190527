// v32_16:  옵저버 패턴을 적용하여 서버를 시작할 때 데이터 로딩, 서버를 종료할 떄 데이터 자장을 수행하자.
...
public class ServerApp {
  ...
  public void execute() {
    ...
    try (ServerSocket serverSocket = new ServerSocket(this.port)) {
      ...
      //서버가 종료 될 때 관찰자(Observer)에게 보고한다.
      for (ServletContextListener listener : listeners) {
  ...
  public void addServeletContextListener(ServletContextListener listener) {
  ...
  public static void main(String[] args) {
    ...
    // =>보고를  받는 객체는 ServeltContextListener 규칙에따라 만든 클래스여야 한다.
    server.addServeletContextListener(new AppInitListener());
    server.execute();
  }
}
