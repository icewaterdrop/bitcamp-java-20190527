// View Resolver 교체하기 - InternalResourceViewResolver 사용하기
...
public class Controller01_2 {
  ...
  public String handler1(Map<String,Object> map) {
    ...
    //     -*.properties 에서 뷰이름에 해당하는 컴포넌트의 URL을 찾는다.
    ...
    //    JSP URL의 접두어와 접미사를 미리 설정해 둘 수 있어 URL을 지정하기 편히한다.
  ...
  public void handler2(Model model) {
    ...
    // 즉 이 핸들러의 URL은 "/c01_2/h2" 이기 때문에 뷰 이름도 "/c01_2/h2" 가된다.
    // InternalResourceViewResolver는 바로 이 URl을 사용하여 다음과 같이 최종 URL을 만든다.
    // => "/WEB-INF/jsp2/" + "/c01_2/h2" + ".jsp" 이다.
...
public ModelAndView handler5() {
  ...
  // 프론트 컨트롤러는 ModelAndView 객체에 보관되어 있는 값들을 
  // ServletRequest 보관소로 옮기고,
  // 뷰 이름을 지정하지 않으면 request handler의 path ViewResolver에게 넘긴다.
  //
  return mv;
}
