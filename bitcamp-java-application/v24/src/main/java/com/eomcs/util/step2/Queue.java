...
public class Queue<E> extends LinkedList<E> implements Cloneable {
  ...
  // 이렇게 클래스안에 선언된 클래스를 nested class(중첩 클래스) 라 부른다.
  // 중첩 클래스 중에서 static이 붙지 않은 중첩 클래스를 "non-static nested class"라 부른다.
  ...
  private class QueueIterator implements Iterator<E> {
    ...
    public boolean hasNext() {
      // 중첩 클래스 안에서는 자신을 생성한 바깥클래스의 인스턴스 주소를 
      // "바깥클래스명.this" 변수에 저장하고 있다.
